            ...
            if (props.mode === 'edit' && selectedTenant.value.treeviews) {
                ...
                if (selectedTenant.value.treeviews.releases) {
                    updateTreeviewNodesCheckedState(
                        ...
                        props.alertTrigger.releases.map(release => release.external_id),
                        'release'
                    );
                }
                if (selectedTenant.value.treeviews.machines) {
                    updateTreeviewNodesCheckedState(
                        ...
                        props.alertTrigger.machines.map(machine => machine.external_id),
                        'machine'
                    );
                }
                if (selectedTenant.value.treeviews.queueDefinitions) {
                    updateTreeviewNodesCheckedState(
                        ...
                        props.alertTrigger.queues.map(queue => queue.external_id),
                        'queue-definition'
                    );
                }
            } else {
...
<template>
    ...
    <FormStep>
        <template #title>
            {{ __('UiPath Orchestrator related entities selection') }}
        </template>

...
const bulkDelete = () => {
    deleting.value = true;
    Inertia.post(route('configuration.automated-processes.bulk-destroy'), {
...
<template>
    <ConfirmationModal :show="confirmingDeletion" @close="close" :closeable="!deleting" severity="error">
        <template #title>
            {{ __('Delete multiple business automated processes') }}
        </template>

        <template #content>
            {{
                __('Are you sure you want to delete selected business automated processes (:number)? Once removed, all of their resources and data will be permanently deleted.', {

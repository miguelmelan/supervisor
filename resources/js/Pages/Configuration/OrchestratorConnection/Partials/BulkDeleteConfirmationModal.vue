...
const bulkDelete = () => {
    deleting.value = true;
    Inertia.post(route('configuration.orchestrator-connections.bulk-destroy'), {
...
<template>
    <ConfirmationModal :show="confirmingDeletion" @close="close" :closeable="!deleting" severity="error">
        <template #title>
            {{ __('Delete multiple UiPath Orchestrator connections') }}
        </template>

        <template #content>
            {{
                __('Are you sure you want to delete selected Orchestrator connections (:number)? Once removed, all of their resources and data will be permanently deleted.', {
